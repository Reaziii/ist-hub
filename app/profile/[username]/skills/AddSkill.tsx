...
const UpdateModal: FC<{ close: () => void, add: (skill: string) => Promise<ServerMessageInterface>, handleRetrive: () => void }> = ({ close, add, handleRetrive }) => {
    return (
        <Formik
            onSubmit={(values, { setSubmitting }) => {
                console.log(add)
                add(values.skill).then((res) => {
                    handleToast(res);
                    setSubmitting(false)
                    handleRetrive();
                })
                                ...
                                <div className="relative p-4 w-full max-h-full">
                                    <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                        ...
                                        <div className='w-[600px] box-border p-5 pt-5'>
                                            ...
                                            <Input name='skill' onChange={handleChange} value={values.skill} error={errors.skill} show={touched.skill ? true : false} placeholder='Senior Nextjs developer' className="w-full box-border" />
...
const AddSkill: FC<{ add: (skill: string) => Promise<ServerMessageInterface>, handleRetrive: () => void }> = ({ add, handleRetrive }) => {
    ...
    return (
        <>
            ...
            {
                open && <UpdateModal close={() => { setOpen(false) }} add={add} handleRetrive={handleRetrive} />
            }
        </>
    )
}
