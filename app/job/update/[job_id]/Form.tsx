...
interface Props {
    update: (params: JobInterface) => Promise<ServerMessageInterface & { job_id?: string }>,
    job_id: string,
    getJobDetails: (job_id: string) => Promise<ServerMessageInterface & { job?: JobInterface }>,
    deleteJob: (job_id: string) => Promise<ServerMessageInterface>
}
...
const UpdateJobForm: React.FC<Props> = ({ update, job_id, getJobDetails, deleteJob }) => {
    ...
    if (!loading && inititalValues._id.length === 0) {
        return <div className='h-[100vh] w-full flex items-center justify-center'>
            <h1>Job Not Found</h1>
        </div>
    }
