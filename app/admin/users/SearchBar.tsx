...
const SearchParametersBar: React.FC<Props> = ({ parameters, setParameters, handleSearch }) => {
    ...
    let depts: ("CSE" | "ECE" | "BBA" | "ALL")[] = ["ALL", "CSE", "ECE", "BBA"];
    let verifieds: ("YES" | "NO" | "ALL")[] = ["ALL", "YES", "NO"]
        ...
        <ul className="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdown-button">

            {
                depts.map((item, key) => (
                    <li key={key}>
                        <button
                            onClick={() => setParameters({ ...parameters, dept: item })}
        ...
        <ul className="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdown-button">

            {
                verifieds.map((item, key) => (
                    <li key={key}>
                        <button
                            onClick={() => setParameters({ ...parameters, verified: item })}
                            type="button" className="inline-flex w-full px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">{item}</button>
                    </li>
                ))
            }
            ...
            <div className="flex relative">
                ...
                <div className="relative w-full">
                    <div className='flex items-center w-full z-[-1]'>
                        ...
                        <button onClick={handleSearch} className="z-1 w-20 relative top-0 h-full end-0 p-2.5 text-sm font-medium p-3.5 text-white bg-blue-700 rounded-e-lg hover:bg-blue-800">
